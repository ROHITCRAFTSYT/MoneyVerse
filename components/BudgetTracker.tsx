...
const BudgetTracker: React.FC<BudgetTrackerProps> = ({ 
  ...
  const availableCategories = type === TransactionType.EXPENSE 
  ...
  // Update category if type switches and current category is invalid
  useEffect(() => {
    ...
    <div className="space-y-6 pb-20">
            ...
            {isAddingCategory ? (
              ...
              <div className="flex gap-2">
                <select
                  ...
                  className={`flex-1 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl py-2 px-4 text-slate-900 dark:text-white focus:outline-none focus:border-verse-accent appearance-none transition-colors font-medium ${type === TransactionType.INCOME ? 'border-emerald-500/30' : 'border-rose-500/30'}`}
                >
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 px-2 animate-fade-in">
          ...
          <div className="flex flex-wrap gap-2 w-full sm:w-auto">
             ...
             >
                ...
                {[...DEFAULT_EXPENSE_CATEGORIES, ...DEFAULT_INCOME_CATEGORIES, ...customCategories].map(cat => (
                    <option key={cat} value={cat}>{cat}</option>
                ))}
             </select>
          </div>
        ...
        {filteredTransactions.length === 0 ? (
          ...
          {[...filteredTransactions].reverse().map((t, index) => (
            <div 
              ...
              className={`
                relative p-4 rounded-2xl border flex justify-between items-center group transition-all duration-500 hover:scale-[1.02] hover:shadow-xl
                ...
                ${t.type === TransactionType.INCOME 
                  ? 'bg-gradient-to-br from-white to-emerald-50/50 dark:from-verse-card dark:to-emerald-900/10 border-emerald-100 dark:border-emerald-900/30 hover:border-emerald-400 dark:hover:border-emerald-500 hover:shadow-emerald-500/10' 
                  : 'bg-gradient-to-br from-white to-rose-50/50 dark:from-verse-card dark:to-rose-900/5 border-slate-200 dark:border-slate-800 hover:border-rose-300 dark:hover:border-rose-500 hover:shadow-rose-500/5'}
              `}
            >
              {/* Left accent indicator with dynamic width */}
              ...
              <div className="flex items-center gap-4 pl-4">
                <div className={`
                  p-3.5 rounded-2xl shadow-sm transition-all duration-500 group-hover:scale-110 group-hover:rotate-6
                  ${t.type === TransactionType.INCOME 
                ...
                <div>
                  <p className="font-bold text-slate-900 dark:text-white text-base transition-colors group-hover:text-verse-accent">{t.description}</p>
                  <div className="flex items-center gap-2 mt-1.5">
                    <span className="text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-lg bg-white/80 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700 shadow-sm transition-all group-hover:border-verse-accent/30 group-hover:text-verse-accent">
                      {t.category}
                    </span>
              ...
              <div className="flex items-center gap-5">
                <div className="text-right">
                  <span className={`font-display font-bold text-xl tracking-tight transition-all duration-300 group-hover:scale-105 inline-block ${t.type === TransactionType.INCOME ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-700 dark:text-slate-200'}`}>
                    {t.type === TransactionType.INCOME ? '+' : '-'}{currencySymbol}{t.amount.toFixed(2)}
                  </span>
                </div>
                
                {/* Actions Container with better positioning */}
                <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                    <button 
                        onClick={() => handleEditClick(t)}
                        className="w-9 h-9 flex items-center justify-center text-slate-400 hover:text-white hover:bg-verse-accent rounded-xl shadow-lg transition-all active:scale-90"
                    ...
                    <button 
                        ...
                        className="w-9 h-9 flex items-center justify-center text-slate-400 hover:text-white hover:bg-rose-500 rounded-xl shadow-lg transition-all active:scale-90"
      ...
      {deleteConfirmationId && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
            <div className="bg-white dark:bg-verse-card w-full max-sm rounded-3xl border border-slate-200 dark:border-slate-700 p-8 shadow-2xl animate-scale-in">
                <div className="flex flex-col items-center text-center mb-8">
                    <div className="w-16 h-16 bg-rose-100 dark:bg-rose-900/30 text-rose-500 rounded-full flex items-center justify-center mb-5 animate-pulse">
                        <Icons.Alert size={32} />
                    </div>
                    <h3 className="text-2xl font-display font-bold text-slate-900 dark:text-white mb-2">Delete This Log?</h3>
                    <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
                        Are you sure? This transaction will be permanently removed from your history and XP cannot be reclaimed.
                    </p>
                </div>
                <div className="flex gap-4">
                    <button 
                        onClick={() => setDeleteConfirmationId(null)}
                        className="flex-1 py-3.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-bold rounded-xl transition-all"
                    >
                        Cancel
                    </button>
                    <button 
                        onClick={confirmDelete}
                        className="flex-1 py-3.5 bg-rose-600 hover:bg-rose-500 text-white font-bold rounded-xl shadow-xl shadow-rose-900/20 transition-all active:scale-95"
                    >
                        Delete
                    </button>
                </div>
